...
unsigned long dribblerTurnOffTime = 1000;
...
enum _tactics {defualt, tactic2}; //REPLACE WITH PROPER NAMES
_tactics tactics = defualt;
...
void loop() {
  ...
  if (attinyData.switches[0] == 255) goalColour = blue;
  else goalColour = yellow;
  ...
  toggle(buttonCode(startButton), options.started);
  // Serial.print("Options, Started: "); Serial.print(options.started);

  if (buttonCode(calibrateButton)) calibrate();
  if (buttonCode(kickerButton)) kick();
  ...
  bluetoothWrite(localBluetoothData, otherBluetoothData);
