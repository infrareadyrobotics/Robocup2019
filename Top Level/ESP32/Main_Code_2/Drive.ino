...
void drive(float Degrees, float targetHeading, int Speed, int rotateAmount) {
  ...
  if(Speed > maxSpeedNearLine){ //Only perform speed limiting if speed is more than speed limit
  ...
  if (Speed > 10) { //If motors are running at reasonable speed run code.
    ...
    if (useHeading < 0) { //Calculate Rotation amount based on heading
      if (haveBall == true) rotate = map(useHeading, -180, 0, 70, 5); //If haveball rotate map diffrently
      else rotate = map(useHeading, -180, 0, maxRotateSpeed, 5); //If not haveball rotate normally
    }
    else if (useHeading > 0) {
      if (haveBall == true) rotate = map(useHeading, 0, 180, -5, -70);
    ...
    */
    ...
    mSpeed[0] += rotate / 100 * rotateSpeed;
    mSpeed[1] += rotate / 100 * rotateSpeed;
    mSpeed[2] += rotate / 100 * rotateSpeed;
    mSpeed[3] += rotate / 100 * rotateSpeed;
  }
