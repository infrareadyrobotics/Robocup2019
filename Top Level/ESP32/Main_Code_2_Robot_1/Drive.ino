...
void drive(float Degrees, float targetHeading, int Speed, int rotateAmount) {
  ...
  if(Speed > maxSpeedNearLine && tactics == normal){ //Only perform speed limiting if speed is more than speed limit and not superteam
  ...
  if (Speed > 10) { //If motors are running at reasonable speed run code.
    ...
    */
    ...
    if (rotateOnSpot == true) {
      // maxRotateSpeed = 150;
      if (useHeading < 0) { //Calculate Rotation amount based on heading
        rotate = map(useHeading, -180, 0, maxRotateSpeed, 5); //If not haveball rotate normally
      }
      else if (useHeading > 0) {
        rotate = map(useHeading, 0, 180, -5, -maxRotateSpeed);
      }
      else rotate = 0;
